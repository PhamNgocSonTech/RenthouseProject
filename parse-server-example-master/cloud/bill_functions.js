...
Parse.Cloud.define('getBill', async req => {
  ...
  queryBill.equalTo('room_Id', room_Id);
  queryBill.include('room_Id.category_Id');
  const objOrder = await queryBill.first();
  console.log('objjjjj', objOrder);
  return objOrder;
});

Parse.Cloud.define('getOrderMine', async req => {
